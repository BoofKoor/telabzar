...
حالتِ remote اجرا می‌کند (کارها همچنان از صفِ Redisِ مستر می‌آیند). سه نقش:
**نودِ دانلود / IP-تمیز** (yt-dlp/gallery-dl/Spotify روی خروجیِ تمیزِ یوتیوب/اینستا)،
**نودِ پردازش / کاهش‌حجم** (opهای سنگینِ CPU مثلِ compress/convert/transcribe/bg روی صفِ
`arq:queue:proc`)، و **نودِ لینک / استریم** (سروِ عمومیِ `/dl` و `/s` روی IPِ تمیز — پروکسیِ
معکوس به gatewayِ مستر روی WG). وقتی نودِ پردازش آنلاین باشد opهای سنگین خودکار به آن سپرده
می‌شوند؛ برای نودِ استریم، در پنل `stream_base` را روی دامنهٔ عمومیِ نود بگذار تا لینک‌ها روی
آن ساخته شوند. نودی نباشد، همه‌چیز روی مستر می‌ماند. ورودیِ پردازش روی HTTP از Bot API گرفته
...
| توزیع‌شده | نودها روی WireGuard (دانلود · پردازش · لینک/استریم) + reaperِ جابِ یتیم | ✅ انجام‌شده |
