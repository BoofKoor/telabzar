...
حالتِ remote اجرا می‌کند (کارها همچنان از صفِ Redisِ مستر می‌آیند). دو نقش:
**نودِ دانلود / IP-تمیز** (yt-dlp/gallery-dl/Spotify روی خروجیِ تمیزِ یوتیوب/اینستا) و
**نودِ پردازش / کاهش‌حجم** (opهای سنگینِ CPU مثلِ compress/convert/transcribe/bg روی صفِ
`arq:queue:proc`). وقتی نودِ پردازش آنلاین باشد، opهای سنگین خودکار به آن سپرده می‌شوند؛
نودی نباشد، همه‌چیز روی مستر می‌ماند. ورودی روی HTTP از Bot API گرفته و خروجی multipart
آپلود می‌شود (`tasks._localize`)؛ `scan` (که به ClamAVِ مستر وصل است) روی مستر می‌ماند.
