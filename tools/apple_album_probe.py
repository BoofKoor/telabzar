...
def target_index(cands: list[dict], track: dict, tol: int = 20) -> int | None:
    ...
    **نسخهٔ اول عضویتِ دقیقِ `_norm` را می‌سنجید و همین ستونِ رتبه را بی‌اعتبار
    کرده بود.** `_norm('Mohammad Noori')` و `_norm('Mohammad Nouri')` برابر
    نیستند، در حالی که `_ratio` بینشان ۹۲٫۹ است — یعنی معیارِ دقیق دقیقاً همان
    تنوعِ رومی‌سازی را رد می‌کرد که کاتالوگِ فارسی پر از آن است. بازتولیدشده:
    ضبطِ درست با املای دیگر (رتبهٔ ۱) رد می‌شد و یک ضبطِ ضعیف‌ترِ دقیق‌املا
    (رتبهٔ ۴) «هدف» می‌شد — همان «جانِ مریم رتبهٔ ۴». و «مرغ سحر پیدا نشد» هم
    همین بود، نه غیبتِ ترک: `Mohammad Reza Shajarian` در برابرِ
    `Mohammadreza Shajarian` دقیق نمی‌خورد ولی ۹۷٫۸ شبیه است، و رتبه‌بندیِ
    تولید هر دو را قبول می‌کرد.

    حالا از `_ARTIST_SAME_MIN`ِ **خودِ تولید** استفاده می‌شود — همان آستانه‌ای
    که تولید برای «این دو نام یک نفرند» دارد؛ ثابتِ تازه‌ای اختراع نمی‌شود.

    محدودیتِ باقی‌مانده، همان که `spotify_query_probe` هم دارد: ریمیکسِ همان
    هنرمند با همان طول واجدِ شرط می‌شود. پس این برای **مقایسهٔ دو شکلِ کوئری
    با هم** است، نه اعلامِ «ضبطِ درست پیدا شد».
    """
    ...
    for i, c in enumerate(cands):
        ...
        if not (dur and cd and abs(cd - dur) <= tol):
            continue
        if any(D._ratio(D._norm(a), w) >= D._ARTIST_SAME_MIN
               for a in D._cand_artists(c) if a for w in want):
            return i
    return None


def final_rank(cands: list[dict], track: dict, idx: int | None) -> int | None:
    """رتبهٔ هدف **بعد از رتبه‌بندی** — تنها عددی که به سؤالِ کوئری جواب می‌دهد.

    رتبهٔ **استخر** آماری نیست که چیزی را ثابت کند: پاک‌سازیِ عنوان استخر را
    بزرگ‌تر می‌کند، پس موقعیتِ خامِ هدف طبعاً عقب می‌رود بدونِ اینکه نتیجه بدتر
    شده باشد. اجرای مستر همین را نشان داد — میانگینِ استخر ۲٫۴ → ۵٫۳ در حالی که
    رتبهٔ نهایی در هر ۷ ترک دست‌نخورده ماند.
    """
    if idx is None:
        return None
    url = D._cand_url(cands[idx])
    ranked = D._rank_candidates(cands, track)
    return next((n for n, (_, c) in enumerate(ranked, 1) if D._cand_url(c) == url), None)
...
async def main() -> int:
    ...
    for tid in ids:
        ...
        for lbl, queries in shapes.items():
            ...
            fr = final_rank(pools[lbl], tr, idx)
            where = ("نیست" if idx is None else
                     f"استخر {idx + 1} → رتبهٔ نهایی {fr}" if fr else
                     f"استخر {idx + 1} → **گیت خورد**")
            print(f"  کوئریِ {lbl:4}: {len(queries)} شکل، {len(pools[lbl])} نامزدِ یکتا   هدف: {where}")
            for q in queries:
                print(f"           {q!r}")
            if fr is not None:
                qrank[lbl].append(fr)
    ...
    for k, v in qrank.items():
        avg = f"{sum(v) / len(v):.2f}" if v else "—"
        print(f"  میانگینِ رتبهٔ **نهاییِ** هدف ({k:4}): {avg}   (n={len(v)})")
    same = sum(1 for a, b in zip(qrank["خام"], qrank["پاک"]) if a == b)
    print(f"  رتبهٔ نهایی در هر دو شکل یکسان ماند: {same}/{min(len(qrank['خام']), len(qrank['پاک']))}")
    print("\n  ⇒ رتبهٔ **استخر** عمداً جمع زده نمی‌شود: پاک‌سازی استخر را بزرگ‌تر می‌کند،")
    print("     پس عقب‌رفتنِ موقعیتِ خام شاهدِ بدترشدن نیست. تنها عددِ معتبر رتبهٔ نهایی است.")
    print("  ⇒ اگر رتبهٔ نهایی دست‌نخورده بماند: پرانتز بی‌ضرر است و پاک‌سازی فقط")
    print("     برای شکلِ دومِ کوئری و بستنِ جریمهٔ کاذب می‌ارزد — مجانی، نه برای کیفیتِ سرچ.")
    return 0
