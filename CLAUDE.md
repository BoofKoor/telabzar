...
- **No node → the master does everything** (verify before changing routing): downloads always have the master's `download-worker` (`arq:queue:dl`); heavy ops route to `arq:queue:proc` only when a processing node is live, and the reaper drains it back otherwise; stream links use `stream_base` **only when a `gateway` node is online** (`ops._link_base` → `role_online("gateway")`), else the master's `public_base`. So a missing/dead node never breaks a path — it falls back to the master.
