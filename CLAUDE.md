...
- **سطلِ کوکیِ خالی «سوخته» نیست — و بیشترِ سطل‌ها ذاتاً خالی‌اند.** `_cookie_platform`
  ...
  **بی‌کوکی** می‌زند (`_warn_cookieless` → `cookieless_used = True` → ادامه، نه `break`) و
  ...
  سطلِ خالی **هیچ رفتاری را عوض نمی‌کند** — با هر دو حالت و با هر دو موتورِ موفق/شکست،
  فراخوانیِ موتور، نبودِ کوکی، ضربه‌ها، DMها و پیامِ کاربر یکسان‌اند؛ تنها تفاوتِ سنجیده‌شده
  یک خطِ لاگ بود. یعنی افزودنشان به `_ANON_FIRST` فقط نویزِ لاگ را کم می‌کند، و
