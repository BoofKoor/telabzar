...
- **یک جابِ `queued`ِ گیرکرده از ۱۷ آگوست ۲۰۲۶ برای همیشه «۱ در صف» نشان
  می‌دهد — رفع نشد، و همین نمونهٔ زندهٔ دلیلِ ساختنِ کارتِ «جابِ گیرکرده» است.**
  `s["queued"]` صرفاً `Job.status == "queued"` را می‌شمارد و هیچ‌چیز جابی را که
  ورکرش مرده تمیز نمی‌کند، پس آن عدد هرگز صفر نمی‌شود و به‌مرور به نویزِ
  دائمی تبدیل می‌شود («همیشه یکی هست» = «هیچ‌وقت نگاهش نکن»). امروز ضرری ندارد
  چون یکی است، ولی دقیقاً همان چیزی را نشان می‌دهد که `/health` باید بگیرد:
  جابی که در صف مانده و سنش از هر `job_timeout`ی گذشته. رفعش دو نیمه دارد و
  هیچ‌کدام در دامنهٔ این PR نبود — یک کارت/شمارنده بر اساسِ `created_at` کهنه،
  و تصمیم دربارهٔ اینکه چنین جابی `failed` علامت بخورد یا دست‌نخورده بماند.
