...
- **ctxِ انتخابگرِ فرمت باید از `YoutubeDL._select_formats` بیاید — رفع شد
  ...
  **`None`** می‌دهند و با ctxِ درست `hls_aac_96k` — دقیقاً همان false failِ
  ...
  `build_format_selector` مقدارِ `ctx['has_merged_format']` را با `[]` می‌خواند نه
  `.get()`. یعنی **کلیدِ غایب بی‌صدا falsy نیست**، که نیمهٔ نانوشتهٔ این تله بود.
