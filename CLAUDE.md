...
- **Node** (optional, remote) — a **download worker on another machine**, joined over WireGuard, consuming the master's `arq:queue:dl` and heartbeating to the master's Redis. Not a new process type: it is `DownloadWorkerSettings` run with `NODE_ROLE` set (`bot.py` flips to `is_local=False`; `worker.py` spawns a heartbeat). Master-side glue in `app/nodes.py` + panel; see §Nodes below.
...
| `app/tasks.py` | `run_op` (ARQ) + `_do_op` op dispatch; live status ticker |
...
| `app/nodes.py` | Distributed **master-side** node layer: `ROLES`, signed one-time WireGuard join token, WG-IP allocation, live registry (Redis `node:{id}` heartbeat, 45 s TTL), WG peer add/remove (config-file + `wg syncconf`), `node_config()` (join reply) |
...
- **Nodes need `is_local=False`**: co-located workers read input straight off the shared Bot API disk (`bot.get_file().file_path`), which does **not** exist on a remote machine. `bot.py` therefore flips to `is_local=False` whenever `NODE_ROLE` is set, so aiogram downloads inputs over HTTP from the (WG-reachable) Bot API and uploads outputs by multipart. A node reaches Redis/Postgres/Bot API only over the WireGuard tunnel (`NODE_*` addresses = master's WG IP); those services must listen on the tunnel. Phase N1 ships only the **download** role (`arq:queue:dl`), which reads nothing from disk — so it works without extra file-transfer plumbing; a future **processing** role (`run_op`) needs that download-input/upload-output path proven first.
