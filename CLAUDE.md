...
- **Main worker** (`arq app.worker.WorkerSettings`) — queue `arq:queue`, runs `run_op`.
...
- **Node** (optional, remote) — a **worker on another machine**, joined over WireGuard, consuming one of the master's queues and heartbeating to the master's Redis. Two roles today: **download** (`arq:queue:dl`, `DownloadWorkerSettings`) and **processing** (`arq:queue:proc`, `ProcessingWorkerSettings` = `run_op` on a dedicated queue). Not a new process type: it is a worker run with `NODE_ROLE` set (`bot.py` flips to `is_local=False`; `worker.py` spawns a heartbeat). Heavy CPU ops route to a live processing node at enqueue time (`ops._op_queue` + `nodes.OFFLOAD_OPS`); no node → everything stays on the master (zero regression). Master-side glue in `app/nodes.py` + panel; see §Nodes below.
...
| `app/nodes.py` | Distributed **master-side** node layer: `ROLES` (download, processing), `OFFLOAD_OPS`, `role_online()`, signed one-time WireGuard join token, WG-IP allocation, live registry (Redis `node:{id}` heartbeat, 45 s TTL), WG peer add/remove (config-file + `wg syncconf`), `node_config()` (join reply) |
...
- Gateway: `PUBLIC_BASE`, `GATEWAY_PORT`, `TLS_CERT`, `TLS_KEY`.
...
- Nodes — master side (one-time WG setup): `NODE_SECRET` (HMAC key for join tokens; falls back to `BOT_TOKEN`), `WG_INTERFACE` (`wg0`), `WG_SUBNET` (`10.51.0.0/24`), `WG_MASTER_IP` (`10.51.0.1`), `WG_MASTER_PUBKEY`, `WG_ENDPOINT` (`host:51820`), `WG_CONFIG_PATH` (`/etc/wireguard/wg0.conf`), and the internal addresses handed to nodes (`NODE_REDIS_URL`, `NODE_POSTGRES_DSN`, `NODE_API_BASE`, `NODE_POT_PROVIDER_URL` — all on the master's WG IP).
