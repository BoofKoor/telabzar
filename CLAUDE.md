...
| `app/cookies.py` | استخرِ اکانتِ کوکی: محتوا روی دیسکِ مستر + آینهٔ Redis (نود)، متادیتا در Redis (`ckmeta:`), وضعیت (`healthy/suspect/invalid/cooldown/disabled`), `pick()` (اولویت + LRU + exclude برای چرخش), `materialize()`, `mark_ok/mark_fail` (کول‌داونِ پلکانی), `healthy_count` |
