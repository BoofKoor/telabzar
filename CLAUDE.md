...
- **کرانِ عددیِ تنظیمات فقط جایی هست که مشتق‌شدنی است — و بقیه عمداً بی‌کران‌اند.**
  ...
  `_UPLOAD_CEILING_MB = 2000` از «~۲۰۰۰ مگابایتِ» `docs/telegram-api.md` (و کامنتِ
  خودِ `config.py` روی `dl_max_size_mb`) که یک تست به همان مستند گره‌اش می‌زند؛ ۱۰۰
  برای دو کلیدی که `config.py` واحدشان را «درصد» اعلام کرده؛ و ۱۰۰ برای `match_min`
  که بازه‌اش در `config.py` نوشته شده. **بقیه سقف ندارند و این تصمیم است نه فراموشی**:
