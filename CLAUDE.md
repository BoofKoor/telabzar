...
می‌دهد. هم‌ردهٔ `cryptography` و مارکرِ `7z`، با این تفاوتِ ظریف که `import
yt_dlp` این‌جا **موفق** است و فقط رفتارِ درونی‌اش فرق می‌کند — پس هیچ
`ModuleNotFoundError`ی هشدار نمی‌دهد و تنها راهِ فهمیدنش خواندنِ فایلِ
requirements است، نه اجرای محلی.
...
- **ردیف‌های یتیمِ کشِ کست‌باکس بی‌خطرند و `DELETE` نمی‌خواهند — برخلافِ اسپاتیفای.**
  ...
  **تفاوتِ باربر با اسپاتیفای:** آن‌جا ردیفِ کهنه **سرو می‌شد** و فایلِ غلط می‌داد،
  ...
  یک جوابِ غلط.
- **هارنسِ انتخابگرِ ساندکلاود `incomplete_formats` را هاردکد می‌کند — باگ نیست،
  تلهٔ نهفته است.** `tests/test_soundcloud_path.py:_picked` در ctx مقدارِ
  `incomplete_formats: False` می‌گذارد و `has_merged_format` را اصلاً نمی‌دهد، در
  حالی که `YoutubeDL._select_formats` هر دو را از خودِ فرمت‌ها می‌سازد و برای
  فیکسچرِ ساندکلاود (که همه‌شان `vcodec="none"`اند) مقدارِ درست **`True`** است.
  **نتیجهٔ #۱۱۵ عوض نمی‌شود** — اجراشده: با هر دو ctx همان `http_mp3_0_0` انتخاب
  می‌شود، چون `ba` بی‌اعتنا به این فلگ جور می‌شود. ولی برای هر انتخابگرِ حاویِ `b`
  روی منبعِ **تک‌نوع** (فقط‌صوتی یا فقط‌ویدیویی) نتیجهٔ غلط می‌دهد: شاخهٔ
  `format_fallback` در `YoutubeDL.py` دقیقاً به آن فلگ گیت خورده. رفعش یک‌خطی است
  و به کست‌باکس ربطی ندارد؛ PRِ جدا.
