...
_KNOWN_UNREACHABLE = {
    # `tasks.py` هندلرش را دارد و `nodes.OFFLOAD_OPS` هم فهرستش کرده و هر دو
    # رشتهٔ `btn_thumb`/`cl_thumb` در دو زبان هست — ولی هیچ ورودی‌ای در
    # `keyboards.OPS_BY_KIND` ندارد، پس هیچ callbackی نمی‌تواند حملش کند.
    # برخلافِ `zip` (که `zip_many` جایش را گرفت) این شکلِ یک **قابلیتِ
    # نیمه‌تمام** را دارد که دکمه‌اش جا مانده، نه بازماندهٔ چیزی. تصمیم با
    # صاحبِ محصول است: یا دکمه اضافه شود یا هندلر برود.
    "thumb",
}
