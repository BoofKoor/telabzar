...
class Settings(BaseSettings):
    ...
    # می‌کند پس در تولید رخ نمی‌داد، ولی پیش‌فرضی که فایل جای اشتباه بنویسد یک
    # مینِ کارگذاشته است — همان شکلی که `.nodes-enabled` و fail-openِ DNS بودند.
    cookies_dir: str = "/cookies"  # پوشهٔ کوکی‌ها (چرخشِ اکانت برای اینستا/X)
    ...
    dl_max_cookie_tries: int = 5   # حداکثر اکانتی که در یک درخواست امتحان می‌شود (۰ = بی‌سقف)
    dl_exit_cooldown_min: int = 45  # وقتی خروجی مقصر شناخته شد، این‌قدر کنار گذاشته می‌شود
    ...
    # آیا موتورِ `direct` هم از PROXY_URL برود؟ روشن = هم‌شکل با پروکسیِ http
    # (که همیشه از پروکسی می‌رفته). خاموش‌کردنش یعنی برگشت به نشتیِ IPِ مستر.
    dl_direct_proxy: bool = True
