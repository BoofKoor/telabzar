...
#: سقفِ **آپلودِ** سرورِ Bot APIِ محلی. مشتق است نه سلیقه — ولی **فقط یک جهت را
#: می‌بندد**، و همین نکته‌ای است که نسخهٔ اولِ این ثابت را غلط کرد.
#:
#: `--local` دو محدودیتِ **متفاوت** دارد، نه یکی (`docs/telegram-api.md`، بندِ
#: baseline، نقلِ مستقیم از READMEِ `tdlib/telegram-bot-api`):
#:
#:   * **دانلود: بدونِ محدودیتِ حجم.**
#:   * **آپلود: تا ۲۰۰۰ مگابایت.**
#:
#: پس این عدد فقط به کلیدی می‌خورد که **بایتِ تازه‌ای را به تلگرام می‌فرستد**.
#: کلیدی که سمتِ **دریافت** است هیچ سقفِ تلگرامی ندارد، و کارتِ یک فایلِ
#: دریافتی هم با `file_id` می‌رود (`cards._media_arg` روی `path=None` رشته
#: می‌دهد نه `FSInputFile`) یعنی صفر بایت آپلود.
#:
...
BOUNDS: dict[str, tuple[int, int | None]] = {
    # ── بایتِ تازه به تلگرام می‌رود → سقفِ آپلود حاکم است ──
    # هر سه با ردیابیِ محلِ خواندن تأیید شدند، نه با شباهتِ نام:
    #   dl_max_size_mb          `tasks_download.py` — چکِ قطعیِ حجمِ روی دیسک
    #                           **پیش از آپلود**، و تحویل با `FSInputFile`.
    #   dl_direct_max_mb        `tasks_download.py` — موتورِ direct؛ اجراشده:
    #                           probe_direct → download_direct → `_spawn`/
    #                           `_deliver_single` → `_media_arg(f, path)` که
    #                           `FSInputFile` می‌دهد. یعنی **آپلود می‌شود**؛
    #                           لینکِ گیت‌وی (`op_link`) مسیرِ دیگری است و روی
    ...
    # را رد می‌کند (`routers/files.py` فقط `size=info.size` را ثبت می‌کند) و نه
    # کارتش بایتی آپلود می‌کند. `--local` دانلود را بی‌سقف می‌کند، پس بستنش روی
    # ۲۰۰۰ مسیری را می‌بندد که در تولید کار می‌کند.
