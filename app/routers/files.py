...
async def on_file(
    ...
    # عکس/ویدیو → قبل از ساختنِ کارت اسکن شود. کارت یعنی ربات فایل را **دوباره
    # آپلود** کرده؛ برای محتوای غیرمجاز همان کار است که نباید انجام شود. اسکن در
    # ورکر است (کارِ CPU نباید حلقهٔ long-pollingِ ربات را بگیرد)، پس این‌جا فقط
    # یک یادداشتِ «در حالِ بررسی» می‌ماند که خودِ ورکر یا کارت می‌کندش یا پیامِ رد.
